...
const CalendarHeader = forwardRef((props: CalendarHeaderProps, ref) => {
  ...
  const renderWeekDays = useMemo(() => {
    ...
    return weekDaysNames.map((day: string, index: number) => {
      ...
      return (
        <Text allowFontScaling={false} key={index} style={{fontSize: 14, textAlign: 'center' ,fontWeight: "600", color: index == 6 ?"#EB6559":"#535252"}} numberOfLines={1} accessibilityLabel={''}>
          {day}
        </Text>
      );
    });
